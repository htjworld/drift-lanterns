...
// Drift Lanterns – TypeScript version
export default function DriftLanternsApp() {
  const [text, setText] = useState<string>(""); // worry text
          ...
          {!started ? (
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="opacity-80">One-minute voyage</span>
...
type LanternLaunchProps = {
  text: string;
  launched: boolean;
};

function LanternLaunch({ text, launched }: LanternLaunchProps) {
  return (
    <div className="pointer-events-none absolute inset-0 z-10 flex items-end justify-center pb-[12rem]">
      <motion.div
        initial={false} // 처음 위치 유지
        animate={
          launched
            ? { y: "-110vh", opacity: [1, 1, 0.9, 0.7, 0.3, 0] } // 떠오르는 애니메이션
            : { y: 0, opacity: 1 } // 제자리에 고정
        }
      ...
      >
        ...
        {!launched && (
          <div className="rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[11px] text-white/70">
...
function Lantern({ lit, text }: LanternProps) {
  return (
...
function AmbientLanterns({ count = 6 }: { count?: number }) {
  // 처음 한 번만 랜덤값을 생성
  const configs = useMemo<LanternConfig[]>(() => {
    ...
    <div className="pointer-events-none absolute inset-0 z-0">
      {configs.map((cfg, i) => (
        ...
        >
          <div
            className="relative"
            style={{ width: cfg.size + 6, height: cfg.size + 12 }}
          >
  ...
  return (
    <div className="pointer-events-none absolute inset-0 opacity-50">
      <div
        ...
        style={{ backgroundImage: stars(160), backgroundRepeat: "repeat" }}
      />
      <div
        className="absolute inset-0"
        style={{
...
function stars(n: number): string {
  const dots = Array.from({ length: n }).map(() => {
    ...
    return `radial-gradient(${s}px_${s}px_at_${x}%_${y}%,rgba(255,255,255,${a}),transparent_${
      s * 2
    }px)`;
  });
  return dots.join(",");
    ...
    <div className="pointer-events-none absolute inset-x-0 bottom-0 h-[36vh]">
      ...
      <div className="absolute inset-x-0 bottom-0 h-[18vh] overflow-hidden">
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse at 50% 120%, rgba(90,120,200,0.35), transparent 60%)",
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse at 50% 120%, rgba(60,100,200,0.22), transparent 55%)",
